...
bool g_exitOnError = false;  // Whether or not ufominer should exit on mining threads errors
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("ufominer - GPU progpow miner");
    ...
    {
        // clang-format off
        cout << "ufominer - GPU ethash miner" << endl
             << "minimal usage : ufominer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default ufominer will try to use all devices types" << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    ufominer provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes ufominer can stay" << endl
                 ...
                 << endl
                 << "    --exit              FLAG Stop ufominer whenever an error is encountered" << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects ufominer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    ufominer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'ufominer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When ufominer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode ufominer tries to auto-detect the correct" << endl
                 ...
                 << "        stratum1    UFO Proxy compatible" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = ufominer_get_buildinfo();
    cout << endl
         << endl
         << "ufominer " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'ufominer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try ufominer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
