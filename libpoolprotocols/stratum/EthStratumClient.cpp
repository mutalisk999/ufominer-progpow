...
void EthStratumClient::connect()
{
    ...
    if (m_conn->HostNameType() == dev::UriHostNameType::Dns || m_conn->HostNameType() == dev::UriHostNameType::Basic)
    {
        ...
        m_resolver.async_resolve(q, m_io_strand.wrap(boost::bind(&EthStratumClient::resolve_handler, this,
...
void EthStratumClient::processResponse(Json::Value& responseObject)
{
    ...
    if (_method == "mining.notify")
    {
        ...
        string input = jPrm.get(Json::Value::ArrayIndex(2), "").asString();

        m_current.block = stoul(jPrm.get(Json::Value::ArrayIndex(4), "").asString(), nullptr, 10);

        string hash_str_no_nonce = "00000000" + prev + "00000000" + input;
    ...
    else if (_method == "mining.set_difficulty")
    {
        ...
        if (jPrm.isArray())
        {
            double nextWorkDifficulty = max(jPrm.get(Json::Value::ArrayIndex(0), 1).asDouble(), 1.0);

            double nextWorkDifficultyAsBtc = nextWorkDifficulty / 256.0;
...
void EthStratumClient::enqueue_response_plea()
{
    ...
    {
        m_response_plea_older.store(response_plea_time.time_since_epoch(), std::memory_order_relaxed);
    }
...
void EthStratumClient::clear_response_pleas()
{
    ...
    m_response_plea_older.store(
        ((steady_clock::time_point)steady_clock::now()).time_since_epoch(), std::memory_order_relaxed);
}
