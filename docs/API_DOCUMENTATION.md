...
```shell
./ufominer [...] --api-bind 127.0.0.1:3333
```
...
```shell
./ufominer [...] --api-bind -3333 --api-password MySuperSecurePassword!!#123456
```

At the time of writing of this document ufominer's API interface does not implement any sort of data encryption over SSL secure channel so **be advised your passwords will be sent as plain text over plain TCP sockets**.

## Usage

Access to API interface is performed through a TCP socket connection to the API endpoint (which is the IP address of the computer running ufominer's API instance at the configured port). For instance if your computer address is 192.168.1.1 and have configured ufominer to run with `--api-bind 3333` your endpoint will be 192.168.1.1:3333.
...
To quickly test if your ufominer's API instance is working properly you can issue this simple command:
...
| [miner_restart](#miner_restart) | Instructs ufominer to stop and restart mining | Yes |
| [miner_reboot](#miner_reboot) | Try to launch reboot.bat (on Windows) or reboot.sh (on Linux) in the ufominer executable directory | Yes
| [miner_getconnections](#miner_getconnections) | Returns the list of connections held by ufominer | No
| [miner_setactiveconnection](#miner_setactiveconnection) | Instruct ufominer to immediately connect to the specified connection | Yes
| [miner_addconnection](#miner_addconnection) | Provides ufominer with a new connection to use | Yes
...
If you get no response or the socket timeouts it's likely your ufominer's instance has become unresponsive (or in worst cases the OS of your mining rig is unresponsive) and needs to be re-started/re-booted.
...
```js
{
  ...
  "result": {
    ...
    "host": {
      "name": "miner01",                                // Host name of the computer running ufominer
      "runtime": 121,                                   // Duration time (in seconds)
      "version": "ufominer-0.18.0-alpha.1+commit.70c7cdbe.dirty"
    },
...
```js
{
  ...
  "jsonrpc": "2.0",
  "result": [
    "ufominer-0.16.0.dev0+commit.41639944", // Running ufominer's version
...
Some of the arguments here expressed have been set for compatibility with other miners so their values are not set. For instance, ufominer **does not** support dual (ETH/DCR) mining.

### miner_restart

With this method you instruct ufominer to _restart_ mining. Restarting means:
...
The invocation of this method **_may_** be useful if you detect one or more GPUs are in error, but in a recoverable state (eg. no hashrate but the GPU has not fallen off the bus). In other words, this method works like stopping ufominer and restarting it **but without loosing connection to the pool**.
...
With this method you instruct ufominer to execute reboot.bat (on Windows) or reboot.sh (on Linux) script which must exists and being executable in the ufominer directory.
As ufominer has no idea what's going on in the script, ufominer continues with it's normal work.
...
which confirms an executable file was found and ufominer tried to start it.

**Note**: This method is not available if the API interface is in read-only mode (see above).

### miner_getconnections

When you launch ufominer you provide a list of connections specified by the `-P` argument. If you want to remotely check which is the list of connections ufominer is using, you can issue this method:
...
* If the selected index is not of an active connection then ufominer will disconnect from currently active connection and reconnect immediately to the newly selected connection
* An error result if the index is out of bounds or the request is not properly formatted

**Please note** that this method changes the runtime behavior only. If you restart ufominer from a batch file the active connection will become again the first one of the `-P` arguments list.

### miner_addconnection

If you want to remotely add a new connection to the running instance of ufominer you can use this this method by sending a message like this
...
Eventually you may want to issue [miner_getconnections](#miner_getconnections) method to identify which is the ordinal position assigned to the newly added connection and make use of [miner_setactiveconnection](#miner_setactiveconnection) method to instruct ufominer to use it immediately.

**Please note** that this method changes the runtime behavior only. If you restart ufominer from a batch file the added connection won't be available if not present in the `-P` arguments list.
...
**Please note** that this method changes the runtime behavior only. If you restart ufominer from a batch file the removed connection will become again again available if provided in the `-P` arguments list.
...
Ethminer, at start, randomly chooses a scramble_nonce, a random number picked in the 2^64 range to start checking nonces from. In addition ufominer gives each GPU a unique, non overlapping, range of nonces called _segment_. Segments ensure no GPU does the same job of another GPU thus avoiding two GPU find the same result.
...
Set the verbosity level of ufominer.
